...
def extract_note_events(midi_path, max_notes=None):
    """
    Извлекает ноты с onset/duration/velocity в секундах.

    Возвращает:
    - список словарей {'pitch', 'onset', 'duration', 'velocity'}
    """
    try:
        mid = MidiFile(midi_path)
        events = []
        ...
        for track in mid.tracks:
            ...
            for msg in track:
                ...
                key = (getattr(msg, "channel", 0), getattr(msg, "note", None))
                if msg.type == 'note_on' and msg.velocity > 0:
                    active_notes[key] = (current_ticks, int(msg.velocity))
                elif msg.type == 'note_off' or (msg.type == 'note_on' and msg.velocity == 0):
                    if key in active_notes:
                        start_ticks, velocity = active_notes.pop(key)
                        onset = mido.tick2second(start_ticks, ticks_per_beat, tempo)
                        end = mido.tick2second(current_ticks, ticks_per_beat, tempo)
                        duration = max(0.05, end - onset)
                        events.append({
                            "pitch": int(msg.note),
                            "onset": float(onset),
        ...
        events.sort(key=lambda x: (x["onset"], x["pitch"]))
        if max_notes and len(events) > max_notes:
...
def create_midi_from_note_events(events, output_path, tempo_bpm=120, track_name="Melody"):
    """
    Создает MIDI из нот с точными onset/duration/velocity.
    """
    timed_events = []
    for event in events:
        timed_events.append({
    ...
    return create_midi_with_precise_timing(timed_events, output_path, tempo_bpm=tempo_bpm)
