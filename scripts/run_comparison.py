...
5. Сохранение CSV/JSON результатов и matched MIDI-фрагментов
"""
import os
import sys
import warnings
...
from src.config import (
    ...
    EEG_MIN_WAVE_DURATION, EEG_MAX_WAVE_DURATION,
    EEG_SCALE_KEY,
    MATCH_FRAGMENT_DURATION, PLAYBACK_TEMPO_MULTIPLIER,
    USE_EMOPIA, EMOPIA_MAX_TRACKS, EMOTION_THRESHOLD,
                        ...
                        for j, cla_win in enumerate(valid_classical_wins):
                            ...
                            if combined_score > best_score:
                                ...
                                best_metrics = {
                                    ...
                                    'classical_window_pitches': cla_win.get('pitches_raw', np.array([])).tolist(),
                                    'classical_window_durations': cla_win.get('durations_raw', np.array([])).tolist(),
                                    'classical_window_ioi': cla_win.get('ioi_raw', np.array([])).tolist(),
                                    'classical_window_velocities': cla_win.get('velocities_raw', np.array([])).tolist(),
                                }
...
def run_comparison(max_participants: int = 2,
    ...
    - window_size: размер окна сравнения в секундах
    - hop_size: шаг окна сравнения в секундах
    - run_id: имя поддиректории в runs/ для сохранения артефактов
    """
    import music21 as m21
    from src.MIDIComparator import extract_window_features
    ...
    comparison_window_size = float(window_size if window_size is not None else COMPARISON_WINDOW_SIZE)
    comparison_hop_size = float(hop_size if hop_size is not None else COMPARISON_HOP_SIZE)
    ...
    print(f"  Window Size: {comparison_window_size:.2f}s")
    print(f"  Hop Size: {comparison_hop_size:.2f}s")
    ...
    if n_jobs > 1 and len(classical_dict) > 3:
        ...
        with ProcessPoolExecutor(max_workers=n_jobs) as executor:
            futures = {
                executor.submit(
                    _process_classical_file,
                    (name, path, comparison_window_size, comparison_hop_size),
                ): name
    ...
    results_df["comparison_window_size"] = comparison_window_size
    results_df["comparison_hop_size"] = comparison_hop_size
    results_df["run_id"] = current_run_id
    ...
    # 5. Подготавливаем артефакты результатов
    print("\n[5/5] Подготовка артефактов результатов...")

    # Prepare per-match MIDI fragments (EEG fragment, classical fragment, combined)
    matches_out_dir = report_dir / "matches"
    matches_out_dir.mkdir(parents=True, exist_ok=True)

    comp_rows = []
    window_size = MATCH_FRAGMENT_DURATION  # Используем длину matched fragment из config
    ...
    for rank, (_, row) in enumerate(top_results.reset_index(drop=True).iterrows(), start=1):
        ...
        # Prepare row for GUI/JSON display export
        comp_rows.append({
            ...
            # Совместимый alias для старых downstream summaries
    ...
    display_json_path = report_dir / "display_results.json"
    display_csv_path = report_dir / "display_results.csv"
    try:
        comp_df.to_json(display_json_path, orient="records", force_ascii=False, indent=2)
        comp_df.to_csv(display_csv_path, index=False)
    except Exception as e:
        print(f"Ошибка сохранения display results: {e}")

    print(f"\n✓ CSV результатов: {results_csv_path}")
    print(f"✓ Display JSON: {display_json_path}")
    print(f"✓ MIDI fragments: {matches_out_dir}")
    print("=" * 60)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--window-size', type=float, default=COMPARISON_WINDOW_SIZE,
                        help='Размер окна сравнения в секундах')
    parser.add_argument('--hop-size', type=float, default=COMPARISON_HOP_SIZE,
                        help='Шаг окна сравнения в секундах')
    parser.add_argument('--run-id', type=str, default=DEFAULT_RUN_ID,
                        help='Имя папки запуска внутри runs/')
    ...
    _cfg.COMPARISON_WINDOW_SIZE = args.window_size
    _cfg.COMPARISON_HOP_SIZE = args.hop_size
